...
var authFolder = storeSystem.fixFileName(`${Helper.opts._[0] || ''}ZiroSessions`)
...
function purgeSession() {
    let prekey = [];
    const directorio = readdirSync('./ZiroSessions');
    const filesFolderPreKeys = directorio.filter((file) => {
        return file.startsWith('pre-key-');
    });
    prekey = [...prekey, ...filesFolderPreKeys];
    filesFolderPreKeys.forEach((files) => {
        unlinkSync(`./ZiroSessions/${files}`);
    });
}
    const directories = ['./ZiroSessions/'];
...
async function connectionUpdate(update) {
  ...
  if (connection === "open") {
       ...
       const infoMsg = `• ᴢɪʀᴏ-ᴍᴅ ᴠ3 ʙᴇʀʜᴀsɪ ᴛᴇʀʜᴜʙᴜɴɢ\n• © ᴢɪʀᴏ ɢᴀɴᴛᴇɴɢ`;
       conn.sendMessage("6281228070013@s.whatsapp.net", {
            text: infoMsg,
